<!--Modal delete team-->
<div class="modal" id="modalDeleteFromStudent-{{$item->id}}" tabindex="-1" role="dialog" aria-labelledby="modalDeleteFromStudentLabel" aria-hidden="true">
    ...
    <div class="modal-content">
      <div class="modal-body border border-2 border-warning rounded-3">
        ...
        <div class="d-flex d-sm-flex d-md-flex d-lg-flex d-xl-flex d-xxl-flex justify-content-center align-items-center justify-content-sm-center align-items-sm-center justify-content-md-center align-items-md-center justify-content-lg-center align-items-lg-center justify-content-xl-center align-items-xl-center justify-content-xxl-center align-items-xxl-center mt-3">
          <h4  class="text-uppercase font-weight-bold text-center">{{ __('admin-dashboard.students-delete-student', ['attribute' => $item->name]) }}</h4>
        </div>
        <div class="d-flex justify-content-center align-items-center mt-3 row">
          <form action="{{ route('admins.students-destroy',$item->id) }}" method="POST">
            @method('DELETE')
            @csrf
            <div class="row">
              <div class="col d-grid gap-2">
                <button class="btn btn-success text-white" type="submit"> {{ __('admin-dashboard.students-dt-btn-modal') }}<i class="fa fa-check"></i></button>
              </div>
              <div class="col d-grid gap-2">
                <button class="btn btn-danger" type="button" data-bs-dismiss="modal" >{{ __('admin-dashboard.students-cancel-btn-modal') }}<i class="fa fa-close"></i></button>
              </div>
            </div>
          </form>
        </div>
